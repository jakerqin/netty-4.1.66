...
public abstract class AbstractBootstrap<B extends AbstractBootstrap<B, C>, C extends Channel> implements Cloneable {
    ...
    public B channel(Class<? extends C> channelClass) {
        // 这个 ReflectiveChannelFactory 就是在 ServerBootstrap.bind中指定用哪一个工厂类来创建Channel
    ...
    final ChannelFuture initAndRegister() {
        ...
        try {
            // 这里是创建Channel，使用的工厂就是在.channel(NioServerSocketChannel.class)中创建的工厂——ReflectiveChannelFactory
            // 其实就是通过反射的方式，把传入的channel反射出来，比如传入的是NioServerScoketChannel或者是NioSocketChannel，它们
        ...
        // 然后再让这个channel注册到Selector上去，关注它的OP_ACCEPT网络事件，不停的轮询它
        // config().group()就是拿到以前创建好的EventLoopGroup
        // 把ServerSocketChannel注册到一个EventLoopGroup上去，意思就是说 让EventLoopGoup中的独立线程采用一个
        // Selector来注册channel，以及轮询事件
